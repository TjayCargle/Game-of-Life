...
Imported.YEP_BankMenuCore = true;

var Yanfly = Yanfly || {};
Yanfly.bank = Yanfly.bank || {};
...
Yanfly.Parameters = PluginManager.parameters('YEP_BankMenuCore');
...
Yanfly.Param.BankYN = eval(String(Yanfly.Parameters['Bank']));
...
Yanfly.bank.DataManager_isDatabaseLoaded = DataManager.isDatabaseLoaded;
DataManager.isDatabaseLoaded = function() {
  if (!Yanfly.bank.DataManager_isDatabaseLoaded.call(this)) return false;
  if (!Yanfly._loaded_YEP_BankMenuCore) {
    this.processShopNotetags($dataItems);
    this.processShopNotetags($dataWeapons);
    this.processShopNotetags($dataArmors);
    Yanfly._loaded_YEP_BankMenuCore = true;
  }
  return true;
};
...
// Game_System
//=============================================================================
Game_System.prototype.initBankSettings = function() {
    this._bank = Yanfly.Param.BankYN;
    
};

Game_System.prototype.isBankEnabled = function() {
    if (this._bank === undefined) this.initBankSettings();
    return this._bank;
};

Game_System.prototype.setBankEnabled = function(value) {
    if (this._bank === undefined) this.initBankSettings();
    this._bank = value;
};
...
Yanfly.bank.Game_Interpreter_pluginCommand =
    Game_Interpreter.prototype.pluginCommand;
Game_Interpreter.prototype.pluginCommand = function(command, args) {
    Yanfly.bank.Game_Interpreter_pluginCommand.call(this, command, args);
    if (command === 'Bank') 
      {$gameSystem.setBankEnabled(true);}
	  else if (command === 'NotBank') {
      $gameSystem.setBankEnabled(false);
    }
...
Window_ShopCommand.prototype.createCommand = function(command) {
	...
	if($gameParty.banking == false)
	 ...
	 this.addCommand("Withdraw", 'buy');
	  ...
	  this.addCommand("Deposit", 'sell', !this._purchaseOnly);
...
//this.addCommand(TextManager.withdraw, 'withdraw');
};
...
Yanfly.bank.Window_ShopBuy_updateHelp = Window_ShopBuy.prototype.updateHelp;
Window_ShopBuy.prototype.updateHelp = function() {
    Yanfly.bank.Window_ShopBuy_updateHelp.call(this);
...
Yanfly.bank.Window_ShopSell_updateHelp = Window_ShopSell.prototype.updateHelp;
Window_ShopSell.prototype.updateHelp = function() {
    Yanfly.bank.Window_ShopSell_updateHelp.call(this);
    if (this._infoWindow) this._infoWindow.setItem(this.item());
    if (this._statusWindow) this._statusWindow.setItem(this.item());
};

Yanfly.bank.Window_ShopSell_isEnabled = Window_ShopSell.prototype.isEnabled;
Window_ShopSell.prototype.isEnabled = function(item) {
    ...
    return Yanfly.bank.Window_ShopSell_isEnabled.call(this, item);
};
...
Yanfly.bank.Window_ShopStatus_initialize = 
    Window_ShopStatus.prototype.initialize;
Window_ShopStatus.prototype.initialize = function(x, y, width, height) {
    Yanfly.bank.Window_ShopStatus_initialize.call(this, x, y, width, height);
...
Yanfly.bank.Window_ShopStatus_update = Window_ShopStatus.prototype.update;
Window_ShopStatus.prototype.update = function() {
    Yanfly.bank.Window_ShopStatus_update.call(this);
...
Yanfly.bank.Window_ShopStatus_paramId = Window_ShopStatus.prototype.paramId;
Window_ShopStatus.prototype.paramId = function() {
    if (Yanfly.Param.ShopStatSwitch) return this._paramId;
    return Yanfly.bank.Window_ShopStatus_paramId.call(this);
};
...
Yanfly.bank.Scene_Shop_onBuyCancel = Scene_Shop.prototype.onBuyCancel;
Scene_Shop.prototype.onBuyCancel = function() {
    Yanfly.bank.Scene_Shop_onBuyCancel.call(this);
...
if ($gameParty.banking == true)
{
if(this._item.oldPrice == null)
{
this._item.oldPrice = this._item.price;
//this._item.price = 0.1;

}

if (this._item.itemType == null)
{
 if (DataManager.isItem(this._item))
 {
this._item.itemType = 0;}
else if(DataManager.isWeapon(this._item))
{this._item.itemType = 1;}
else if (DataManager.isArmor(this._item))
{this._item.itemType = 2;}
 else
{this._item.itemType = 4;}
 } 

 var amount = number;
 
for (i = 0; i < $gameParty._storedItems.length; i++) 
//for (i = $gameParty._storedItems.length; i >= 0; i--) 
{

//$gameMessage.add(this._item.itemType.toString());
// if ($gameParty._storedItems[i].toString() == [this._item.itemType,this._item.id,1,this._item.price].toString()){
	
	 //for (k = 0; k < $gameParty._storedItems.length; k++) 
	 //for (i = 0; i < $gameParty._storedItems.length; i++) 
	  if ($gameParty._storedItems[i].toString() == [this._item.itemType,this._item.id,1,this._item.price].toString())
	  {
		if(amount > 0)
		{
	    $gameParty.unStore(i,[this._item.itemType,this._item.id,1,this._item.price]);
	...
	// }
	 ...
	 if(this._item.oldPrice != null)
{
this._item.price = this._item.oldPrice;
...
if (DataManager.isItem(this._item)) {
 ...
 //this._goods.push([0,this._item.id,1,this._item.price]); 
 for (j = 0; j < number; j++) 
  $gameParty.store( [0,this._item.id,1,this._item.price] );
 };
 if (DataManager.isWeapon(this._item)) {

 //this._goods.push([1,this._item.id ,1,this._item.price]) ;
 for (j = 0; j < number; j++) 
   $gameParty.store( [1,this._item.id,1,this._item.price] );
 };
 if (DataManager.isArmor(this._item)){
 
 //this._goods.push([2,this._item.id,1,this._item.price]);
 for (j = 0; j < number; j++) 
 $gameParty.store( [2,this._item.id,1,this._item.price] );
...
Yanfly.bank.Scene_Shop_activateSellWindow = 
    Scene_Shop.prototype.activateSellWindow;
Scene_Shop.prototype.activateSellWindow = function() {
    Yanfly.bank.Scene_Shop_activateSellWindow.call(this);
    this._statusWindow.show();
};

Yanfly.bank.Scene_Shop_onSellCancel = Scene_Shop.prototype.onSellCancel;
Scene_Shop.prototype.onSellCancel = function() {
    Yanfly.bank.Scene_Shop_onSellCancel.call(this);
    this._infoWindow.setItem(null);
};
