...
Game_Action.prototype.applyItemUserEffect = function(target) {
    ...
    this.subject().gainSilentTp(0);//value);
};
...
Object.defineProperties(Game_BattlerBase.prototype, {
	...
	// Tp Cost Rate
    tcsr: { get: function() { return this.sparam(10); }, configurable: true },
    // Tp Charge Rate
...
Game_BattlerBase.prototype.immortalStateId = function() {
    return 3;
};
...
Game_BattlerBase.prototype.skillTpCost = function(skill) {
    return Math.floor(skill.tpCost * this.tcsr) ;
};



Game_BattlerBase.prototype.canPayMPCost = function(skill) {
...
Game_Party.prototype.initAllItems = function() {
    this._items = {};
	this._items.oldPrice =this._items.price 
	this._items.itemType = null;
    this._weapons = {};
    this._armors = {};
...
Game_Party.prototype.gainItem = function(item, amount, includeEquip) {
    ...
    if (container) {
		...
		if (item.mMax == null)
		{item.mMax = 0};
        container[item.id] = newNumber.clamp(0, this.maxItems(item) + (item.mMax));
...
Game_Party.prototype.store = function(item) {

     this._storedItems.push(item);
	 
};


Game_Party.prototype.sortItems = function(item) {

     this._storedItems.push(item);
};


Game_Party.prototype.unStore = function(index,item) {
...
Game_Interpreter.prototype.command104 = function() {
    if (!$gameMessage.isBusy()) {
		//$gameMessage.add(this._params.toString());
...
Game_Interpreter.prototype.command302 = function() {
    if (!$gameParty.inBattle()) {
	    var goods2 = $gameParty._storedItems;//[this._params];
        var goods = [this._params];
        while (this.nextEventCode() === 605) {
            this._index++;
            goods.push(this.currentCommand().parameters);
			
			//goods.push([type,index,EditPrice Y/N, Price]);
        }
		if(goods == [0,50,0,0,false].toString())
		{
		$gameParty.banking = true;
		 SceneManager.push(Scene_Shop);
        SceneManager.prepareNextScene(goods2, this._params[4]);
		}
		else
		{
		$gameParty.banking = false;
		 SceneManager.push(Scene_Shop);
